...
from src.models.github_events import GitHubIssue, GitHubWebhookPayload
...
async def handle_github_webhook(
    ...
    payload = GitHubWebhookPayload.from_dict(payload_data)
    ...
    if event_type == "issues" and payload.action == "opened" and payload.issue:
        ...
        if "[Application Plan]" in issue.title or "agent:plan" in labels:
            if not payload.repository:
                return {"status": "error", "reason": "Repository info missing"}
...
def root() -> dict[str, str]:
    ...
    Returns:
        A welcome message.
    """
    return {"message": "OS-APOW Event Notifier", "docs": "/docs"}
